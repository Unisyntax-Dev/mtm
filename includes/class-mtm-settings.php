...
class MTM_Settings {
    ...
    public function register_settings() {
        ...
        add_settings_section(
            'mtm_main_section',
            __('Mini Task Manager Settings', 'mini-task-manager'),
            function () {
        ...
        add_settings_field(
            'allow_public_submit',
            __('Allow public submit', 'mini-task-manager'),
            [$this, 'field_allow_public_submit'],
            'mtm_settings_page',
            'mtm_main_section'
        );
        ...
        add_settings_field(
            'title_max_length',
            __('Title max length', 'mini-task-manager'),
            [$this, 'field_title_max_length'],
            'mtm_settings_page',
            'mtm_main_section'
        );
    }
    ...
    public function sanitize_settings($input): array {
        ...
        $out['allow_public_submit'] = !empty($input['allow_public_submit']) ? 1 : 0;
        $out['enable_delete']       = !empty($input['enable_delete']) ? 1 : 0;
        $out['title_max_length']    = isset($input['title_max_length']) ? max(1, min(255, (int)$input['title_max_length'])) : $d['title_max_length'];
    ...
    public function field_allow_public_submit() {
        $o = $this->get_option();
        printf(
            '<label><input type="checkbox" name="%1$s[allow_public_submit]" value="1" %2$s /> %3$s</label>',
            esc_attr(self::OPTION),
            checked(!empty($o['allow_public_submit']), true, false),
            esc_html__('Allow adding tasks without login (for demo/testing)', 'mini-task-manager')
        );
    }
    ...
    public function field_title_max_length() {
        $o = $this->get_option();
        printf(
            '<input type="number" name="%1$s[title_max_length]" value="%2$d" min="1" max="255" class="small-text" />',
            esc_attr(self::OPTION),
            (int)$o['title_max_length']
        );
        echo ' <span class="description">' . esc_html__('Max allowed title length (1–255).', 'mini-task-manager') . '</span>';
    }
